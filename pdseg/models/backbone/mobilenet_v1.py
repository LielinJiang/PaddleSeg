...
class MobileNetV1():
    def __init__(self, output_stride=32, scale=1.0):
        self.scale = scale
        self.output_stride = output_stride

    def net(self, input, class_dim=1000, end_point=-1):
        scale = self.scale
        layer_cnt = 0
        # conv1: 112x112
        ...
        layer_cnt = 23
        if layer_cnt == end_point:
            return input, decode_short
        s = 1 if self.output_stride == 16 else 2
        # 7x7
        input = self.depthwise_separable(
            input,
            num_filters1=512,
